...
describe("AuthService", () => {
  ...
  describe("login", () => {
    ...
    it("throws ServiceUnavailableException when maintenance mode is on for a non-super-admin", async () => {
      ...
      platformSettingRepo.find.mockResolvedValue([
        { maintenance_mode: true, maintenance_message: "Down for maintenance" },
      ]);
    ...
    it("allows SUPER_ADMIN to log in even during maintenance mode", async () => {
      ...
      platformSettingRepo.find.mockResolvedValue([
        { maintenance_mode: true, maintenance_message: "Down" },
      ]);
  ...
  describe("reissueTokensForUser", () => {
    ...
    it("throws NotFoundException when the user does not exist", async () => {
      ...
      await expect(service.reissueTokensForUser("missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("verifyOtp", () => {
    ...
    it("throws BadRequestException when there is no outstanding OTP", async () => {
      ...
      await expect(
        service.verifyOtp({ email: "user@example.com", code: "123456" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("increments attempts and throws when the code is incorrect", async () => {
      ...
      await expect(
        service.verifyOtp({ email: "user@example.com", code: "000000" }),
      ).rejects.toThrow(BadRequestException);
      expect(otpRepo.save).toHaveBeenCalledWith(
    ...
    it("respects maintenance mode for non-super-admin users", async () => {
      ...
      platformSettingRepo.find.mockResolvedValue([
        { maintenance_mode: true, maintenance_message: null },
      ]);
  ...
  describe("requestPasswordReset", () => {
    ...
    it("returns the generic response (without a new send) when a code was requested recently", async () => {
      usersRepo.findOne.mockResolvedValue({ ...baseUser });
      otpRepo.createQueryBuilder.mockReturnValue(
        buildQueryBuilder({ id: "existing-otp" }),
      );
  ...
  describe("changePassword", () => {
    ...
    it("throws NotFoundException when the user does not exist", async () => {
      ...
      await expect(
        service.changePassword("missing", { newPassword: "newpass123" } as any),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("throws BadRequestException when currentPassword is required but missing", async () => {
      ...
      await expect(
        service.changePassword("user-1", { newPassword: "newpass123" } as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("tableGuestLogin", () => {
    ...
    it("throws NotFoundException when the table does not exist", async () => {
      ...
      await expect(service.tableGuestLogin("missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
