...
describe('AppApkReleasesService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [AppApkReleasesService, { provide: getRepositoryToken(AppApkRelease), useValue: repo }],
    }).compile();
  ...
  describe('getLatest', () => {
    it('returns the latest active release when its file exists (Object Storage URL, always trusted)', async () => {
      repo.findOne.mockResolvedValue({ version_name: '1.2.0', apk_url: 'https://x/app-releases/o.apk', checksum: 'abc', notes: 'fix' });

      const result = await service.getLatest('android');

      expect(result).toEqual({ versionName: '1.2.0', url: 'https://x/app-releases/o.apk', checksum: 'abc', notes: 'fix' });
    });
    ...
    it('returns null for a legacy /uploads release whose file no longer exists on disk', async () => {
      repo.findOne.mockResolvedValue({ version_name: '1.0.0', apk_url: '/uploads/apks/old.apk', checksum: 'x', notes: null });
  ...
  describe('getDownloadUrl', () => {
    it('returns the latest release url when available', async () => {
      repo.findOne.mockResolvedValue({ apk_url: 'https://x/app-releases/o.apk' });

      expect(await service.getDownloadUrl('android')).toBe('https://x/app-releases/o.apk');
    });

    it('falls back to the hard-coded public URL when there is no active release', async () => {
      repo.findOne.mockResolvedValue(null);

      expect(await service.getDownloadUrl('android')).toBe('https://obix-apk-download.vercel.app/obix.apk');
    });
  ...
  describe('create', () => {
    it('uploads the apk and creates an active release', async () => {
      const file = { buffer: Buffer.from('apk-data'), originalname: 'app.apk', mimetype: 'application/vnd.android.package-archive' };

      const result = await service.create(file, { platform: 'android', versionName: '2.0.0' });

      expect(sendMock).toHaveBeenCalled();
      expect(repo.create).toHaveBeenCalledWith(expect.objectContaining({ version_name: '2.0.0', is_active: true }));
      expect(result).toBeDefined();
    });

    it('throws BadRequestException when no file is provided', async () => {
      await expect(service.create(null, { platform: 'android', versionName: '2.0.0' })).rejects.toThrow(BadRequestException);
    });
