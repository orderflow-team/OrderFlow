...
export default function SignupPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post('/auth/signup/otp', { email: email.trim().toLowerCase() });
        ...
        <div className={`rounded-[2.5rem] bg-white/25 backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-white/50 p-7 ${GLASS_SHEEN}`}>
          {step === 'details' ? (
            <>
              ...
              <p className="text-slate-600 font-medium text-sm mt-1 mb-6">Enter your details to receive an email verification code.</p>
            ...
            <form onSubmit={handleVerifySignup} className="space-y-4">
              <h2 className="text-2xl font-bold tracking-tight text-slate-800">Verify email</h2>
              ...
              <Input
                ...
                maxLength={6}
                autoFocus
                className={`text-center text-xl font-mono tracking-widest ${GLASS_INPUT} focus-visible:ring-2 focus-visible:ring-orange-400/70 focus-visible:bg-white/55`}
              />
              ...
              <div className="flex items-center justify-between pt-2 text-sm">
                ...
                <button
                  ...
                  className={`font-semibold ${
                    cooldown > 0 ? 'text-slate-400 cursor-not-allowed' : 'text-orange-600 hover:text-orange-700'
                  }`}
                >
