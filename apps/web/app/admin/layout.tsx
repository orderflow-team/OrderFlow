...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`${theme === 'dark' ? 'dark' : ''} min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-indigo-500 selection:text-white`}>
      {/* Top Bar */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 pl-3 border-l border-border">
            ...
            <div className="hidden lg:block text-left">
              ...
              <div className="text-[10px] text-indigo-600 dark:text-indigo-400 font-mono">{user?.role?.toUpperCase() || 'SUPER_ADMIN'}</div>
            </div>
          </div>
        </div>
      </header>
        ...
        >
          ...
          <div className="pt-4 border-t border-border">
            <div className="p-3 bg-muted rounded-xl border border-border flex items-center gap-3">
              <ShieldAlert className="w-5 h-5 text-indigo-600 dark:text-indigo-400 flex-shrink-0" />
